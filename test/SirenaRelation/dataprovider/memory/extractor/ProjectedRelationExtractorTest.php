...
class ProjectedRelationExtractorTest extends \PHPUnit_Framework_TestCase implements DataProviderInterface
{
	private $relation;
	private $data;
	...
	public function testCanHandle() {
		$extractor = new ProjectedRelationExtractor();

		$this->assertFalse($extractor->canHandle($this->relation));
		$this->assertTrue($extractor->canHandle($this->relation->project(array('attributeA'))));
	}
	...
	public function testReadFromABaseRelationReturnAnArrayObjectContaininTheProjectedColumns() {
		$extractor = new ProjectedRelationExtractor();
		$iterator  = $extractor->read($this->relation->project(array('attributeA','attributeC','attributeE')));
		$expectedData = array(
								array("1,1", '1,3', "1,5"),
								array("2,1", '2,3', "2,5"),
								array("3,1", '3,3', "3,5"),
